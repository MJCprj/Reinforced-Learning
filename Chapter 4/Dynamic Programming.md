...
2. 반복 정책 평가(Iterative policy evaluation)
   ...
   > $v_{k+1}(s)\doteq\displaystyle{\mathbb{E}_{\pi}[R_{t+1}+\gamma v_{k}(S_{t+1})|S_{t}=s]}$
...
1. 정책 향상 정리(Policy improvement theorem)
   - 기본 전제
      ...
      > $q_\pi(s, a)\doteq\mathbb{E}[R_{t+1}+\gamma v_{\pi}(S_{t+1})|S_{t}=s, A_{t}=a]$
...
2. 정책 향상(Policy improvement)
   ...
   > $\pi'(s) \doteq \displaystyle{\argmax_{a}q_{\pi}(s, a)}$
   = $\displaystyle{\argmax_{a}\mathbb{E}[R_{t+1}+\gamma v_{\pi}(S_{t+1})|S_{t}=s, A_{t}=a]}$
   ...
   > $v_{\pi'}(s) = \displaystyle{\max_{a}\mathbb{E}[R_{t+1}+\gamma v_{\pi'}(S_{t+1})|S_{t}=s, A_{t}=a]}$
...
>   - 정책 $\pi_{0}$에 대하여 상태 가치 함수 $v_{\pi_{0}}$를 계산한다.
   - 가치 함수 $v_{\pi_{0}}$를 이용해 정책 $\pi_{0}$를 더 좋은 정책  $\pi_{1}$으로 향상시킨다.
   - 정책 $\pi_{1}$에 대하여 상태 가치 함수 $v_{\pi_{1}}$을 계산한다.
   - 가치 함수 $v_{\pi_{1}}$을 이용해 정책 $\pi_{1}$을 더 좋은 정책  $\pi_{2}$로 향상시킨다.
   ...
   - 정책 $\pi_{n}$에 대하여 상태 가치 함수 $v_{\pi_{n}}$을 계산한다.
...
2. 가치 반복
   - 정책 반복의 과정에서 정책 평가 단계의 반복 계산 과정을 중간에 멈출 수 있는 방법 중 하나
   ...
   > $v_{k+1}(s)\doteq\displaystyle{\max_{a}\mathbb{E}[R_{t+1}+\gamma v_{k}(S_{t+1})|S_{t}=s, A_{t}=a]}$
